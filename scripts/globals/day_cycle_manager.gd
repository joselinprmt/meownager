...
const GAME_MINUTE_DURATON: float = TAU / MINUTES_PER_DAY

var game_speed: float = 5.0
var initial_day: int = 1
var initial_hour: int = 7
var initial_minute: int = 01

var time: float = 0.0
var current_minute: int = -1
var current_day: int = 0
var break_day: int = 14
var result_showed: bool = false
...
func _process(delta: float) -> void:
	time += delta * game_speed * GAME_MINUTE_DURATON
	game_time.emit(time)
...
func set_initial_time() -> void:
	...
	time = initial_total_minutes * GAME_MINUTE_DURATON
	

func recalculate_time() -> void:
	var total_minutes: int = int(time / GAME_MINUTE_DURATON)
	...
	if current_day != day:
		...
		if day >= break_day and !result_showed:
			show_results()
...
func show_results():
	set_process(false)
	result_showed = true
	var result_panel = preload("res://scenes/ui/result_panel.tscn").instantiate()
